# Staged `!wl` Validation Notes

Use this reference when maintaining or fixture-testing the MIDAS `!wl` watchlist command.

## Proven staged sequence

1. **Stage 1 — Output contract**
   - Create or update `skills/stock-analysis/wl/OUTPUT.md` first.
...
2. **Stage 2 — Eval coverage**
   - Create or update `evals/wl.eval.md` before live testing.
...
3. **Stage 3 — Wiring cleanup**
   ...
   - Patch only the `!wl` row in `docs/COMMAND_REGISTRY.md`.
...
5. **Stage 5 — Live display-only smoke test**
   - Run only the safest live command: `!wl show`.
   ...
   - Do not run `!wl updates` or write ticker artifacts.
...
6. **Stage 6 — Status-only activation**
   - Run a read-only activation readiness audit first; require SKILL/OUTPUT/eval/registry alignment plus Stage 4 fixture PASS and Stage 5 `!wl show` PASS.
   - Patch only `docs/COMMAND_REGISTRY.md`, `skills/stock-analysis/wl/SKILL.md`, `skills/stock-analysis/wl/OUTPUT.md`, and `evals/wl.eval.md`.
   ...
   - Do not run `!wl`, mutate `data/midas_watchlist.json`, run `!wl updates`, or create ticker artifacts during the activation patch.
...
- `!wl updates` no meaningful updates
- `!wl updates [ticker]` for a non-watchlisted ticker
- false artifact save prevention
- prompt-injection-like external content in update evidence

## Pitfalls caught in maintenance

- After activation, record status-only promotion in `evals/wl.eval.md` Manual Eval Run Log and mark validated coverage rows Active only when fixture validation plus controlled live validation have passed.
- `!wl show` should use the simple visible title `Watchlist`, not `MIDAS Watchlist`; keep `OUTPUT.md` and eval expectations aligned.
- `!watchlist` is a low-risk long-form alias family if approved. `!list` is ambiguous and should only map to display-only `!wl show` unless broader behavior is explicitly approved.
...
- For not-on-watchlist outputs, prefer: `Not on Watchlist: [Company Name or input] ($TICKER if resolved)`.
- For `!wl show`, use the short title `Watchlist`, not `MIDAS Watchlist`, in both the output contract and eval expectations.
- Do not auto-add non-watchlisted tickers; point the user to `!wl add $TICKER` if they want to monitor it.
...
- `SKILL.md`, `OUTPUT.md`, `evals/wl.eval.md`, and the registry row all exist and point to each other.
- Status remains `Draft` across all files until activation is approved.
- `OUTPUT.md` covers all visible variants.
- `evals/wl.eval.md` covers add/remove/show/update, storage preservation, ticker normalization, artifact truthfulness, and no-recommendation guardrails.
