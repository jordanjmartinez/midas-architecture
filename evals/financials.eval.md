...
- If user asks for a complete packet: route to `!full [ticker]`.
...
- Auto-running `!full`, `!thesis`, `!risk`, `!research`, or `!earnings`
...
- Appropriate routing hint such as `!full HOOD` if the user wants a complete packet
...
`full` must not activate Full mode. If the user wants a complete packet, recommend `!full ONON` without auto-running it. If the request remains financial-statement scoped, normal Standard `!financials` may run using the required Standard section order.
...
- Recommendation to use `!full ONON` for a complete packet when appropriate.
- No saved-path claim.
